...
// RF module — present whenever board defines RF_RX_GPIO (e.g. esp32c3_rfbot)
#ifdef RF_RX_GPIO
#include "rf.h"
#endif
...
void app_main(void) {
    ...
    // 433 MHz RF module (TX + RX task)
#ifdef RF_RX_GPIO
    rf_module_init();
    rf_start_receiver();
    ESP_LOGI("MAIN", "RF module initialised (TX=GPIO%d, RX=GPIO%d)", RF_TX_GPIO, RF_RX_GPIO);
#endif
