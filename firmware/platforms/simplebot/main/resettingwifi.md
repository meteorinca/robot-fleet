# Resetting SimpleBot's WiFi Settings

Use this procedure to erase all saved WiFi networks and return SimpleBot to hotspot mode.

---

## When to do this

- You want to remove all the saved WiFi networks from SimpleBot.
- SimpleBot can't connect to any known network and you want to start fresh.
...
3. After restart, SimpleBot broadcasts its own WiFi hotspot (`SimpleBot-<number>`) so you can set it up fresh via the web portal.
...
1. SimpleBot restarts (takes ~3 seconds).
2. A new WiFi hotspot named **`SimpleBot-<N>`** appears (where `<N>` is the device number).
...
6. SimpleBot restarts and connects to the new network 🎉
