...
static void led_heartbeat_task(void *pvParameters) {
    ...
    while (1) {
...
#if IS_FLASHLIGHT
        // Flashlight mode: do not run ambient breathing PWM; remain in manual state
        ...
        if (connected) {
            ...
            bool on = false;
            if      (cycle_ms < 100) on = true;
            else if (cycle_ms < 250) on = false;
            else if (cycle_ms < 350) on = true;
            else                     on = false;
            led_set(on);
        }
#endif
